...
ui <- dashboardPage(skin = "red",
         ...
         dashboardBody(
             tabItems(tabItem(tabName = "help",
                              ...
                              "It is sometimes desirable to power a study on the precision of an estimate rather than for a particular hypothesis test.", tags$code("presize"), "provides a range of functions for performing these calculations. ", tags$code("presize"), "returns either the confidence interval width that could be expected given a sample size, or the sample size that would be necessary to attain a given confidence interval width.",
                              tags$br(),
                              "For instance, it may be that we want to estimate the mean amount of a blood parameter to within 5 units. Based on published literature, we expect the mean value to be 20 units, with a standard deviation of 3. To achieve the 5 unit confidence interval width,", textOutput("ex_n", inline = TRUE), "participants are required. If we know that we have funding to include 50 participants, we can calculate the confidence interval width that we could expect, and find that it would be", textOutput("ex_ciw", inline = TRUE), "units wide.",
                              tags$br(),
                              tags$br(),
                              ...
                              tags$br(),
                              tags$br(),
             "This site uses the R ", tags$code("presize"), " R package", " (version ", textOutput("version", inline = TRUE),"), which was developed at CTU Bern, the Clinical Trials Unit of the University of Bern and University Hospital Bern, which is part of the ", tags$a(href = "https://www.scto.ch", "Swiss Clinical Trials Orgnization"), ". The R package version of ", tags$code("presize"), " can be installed in R using the ", tags$code("remotes"), "package via ", tags$code("remotes::install_github('ctu-bern/presize')"), ". The R code for running the calculations in this site is shown after the results."),
